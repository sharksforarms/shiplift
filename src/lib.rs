...
use rep::{Change, Container as ContainerRep, ContainerCreateInfo,
          ContainerDetails, Event, Exit, History, Image as ImageRep,
          ImageDetails, Info, NetworkCreateInfo,
          NetworkDetails as NetworkInfo, SearchResult, Stats, Status, Top,
          Version};
...
impl<'a, 'b> Image<'a, 'b> {
    ...
    pub fn delete(&self) -> Result<Vec<Status>> {
        ...
        match ::serde_json::from_str(&raw)? {
            Value::Array(ref xs) => xs.iter()
                .map(|j| {
                    let obj = j.as_object().ok_or_else(|| {
                        EK::JsonTypeError("<anonym>", "Object")
                    })?;
                    ...
                    if let Some(sha) = obj.get("Untagged") {
                        sha.as_str()
                            ...
                            .ok_or_else(|| {
                                EK::JsonTypeError("Untagged", "String")
                            })
                    } else {
                        obj.get("Deleted")
                            .ok_or_else(|| {
                                EK::JsonFieldMissing("Deleted' or 'Untagged")
                            })
                            .and_then(|sha| {
                                sha.as_str()
                                    .map(|s| Status::Deleted(s.to_owned()))
                                    .ok_or_else(|| {
                                        EK::JsonTypeError("Deleted", "String")
                                    })
                            })
                    }
                })
...
impl<'a> Images<'a> {
    ...
    pub fn search(&self, term: &str) -> Result<Vec<SearchResult>> {
        ...
        let raw = self.docker
            .get(&format!("/images/search?{}", query)[..])?;
...
impl<'a, 'b> Container<'a, 'b> {
    ...
    pub fn stats(&self) -> Result<Vec<Stats>> {
        self.docker
            ...
            .and_then(|r| {
                ::serde_json::from_reader::<_, Vec<Stats>>(r)
                    .map_err(Error::from)
            })
    }
    ...
    pub fn stop(&self, wait: Option<Duration>) -> Result<()> {
        ...
        if let Some(w) = wait {
            let encoded = form_urlencoded::serialize(vec![(
                "t",
                w.as_secs().to_string(),
            )]);
            path.push(encoded);
            ...
            .post(s, Some((&mut bytes, ContentType::json())))
        {
            ...
            Ok(res) => {
                ...
                if let Value::Object(ref _obj) = json {
                    ...
                    Err(Error::from_kind(EK::JsonTypeError(
                        "<anonymous>",
                        "Object",
                    )))
                }
...
impl Docker {
    ...
    pub fn host(host: Url) -> Result<Docker> {
        match host.scheme() {
            ...
            _ => {
                let client = if let Some(ref certs) =
                    env::var("DOCKER_CERT_PATH").ok()
                {
    ...
    pub fn events(&self, opts: &EventsOptions) -> Result<Vec<Event>> {
        ...
        self.stream_get(&path.join("?")[..])
            .and_then(|r| {
                ::serde_json::from_reader::<_, Vec<Event>>(r)
                    .map_err(Error::from)
            })
    }
    ...
    {
        self.transport
            .stream(Method::Post, endpoint, body)
    }
